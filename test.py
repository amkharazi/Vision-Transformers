...
def main():
    ...
    def eval_weights_file(weights_path: str) -> str:
        model = build_model(
            args.model_type, args.num_classes, tensor_kwargs, img_shape
        ).to(device)
        state = torch.load(weights_path, map_location=device)
        model.load_state_dict(state, strict=False)
        top1, top2, top3, top4, top5, loss, elapsed = evaluate_model(
            model, test_loader, device
        )
        report = (
            f"{args.model_type} | test | weights={os.path.basename(weights_path)} | "
