...
# Function to get label names for all
...
# Function to get label names for training
...
def get_tinyimagenet_dataloaders(data_dir='../datasets', transform_train=None, transform_val=None, transform_test=None, batch_size=64, image_size=192):
    ...
    # Split the training dataset into new training and temporary test datasets
    train_size = 70000
    temp_test_size = 30000
    dataset_train, dataset_temp_test = random_split(dataset_train, [train_size, temp_test_size])

    dataset_val = ValDataset(root_dir=data_dir, transform=transform_val)

    # Combine the temporary test dataset with the validation dataset to get 40,000 images for the test set
    combined_test_dataset = ConcatDataset([dataset_temp_test, dataset_val])
    test_size = 40000
    _, dataset_test = random_split(combined_test_dataset, [len(combined_test_dataset) - test_size, test_size])
