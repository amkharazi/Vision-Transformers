...
if __name__ == '__main__':
    ...
    weights_path = os.path.join('../results',TEST_ID, 'model_stats', f'Model_epoch_{start_epoch}.pth')
    ...
    with open(os.path.join(result_dir, 'model_stats', 'model_info_continue.txt'), 'a') as f:
        f.write(f'total number of parameters:\n{num_parameters}')

    # Train from Scratch - Just Train
    print(f'Training for {len(range(n_epoch))} epochs\n')
    for epoch in range(start_epoch,n_epoch+start_epoch+1):
        ...
        with open(os.path.join(result_dir, 'accuracy_stats', 'report_train_continue.txt'), 'a') as f:
            f.write(report)
