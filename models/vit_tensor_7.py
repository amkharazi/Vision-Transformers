...
# different embedding on each mode per block
...
class Attention(nn.Module):
    def __init__(self, mode, dim, num_patches, num_heads=(3,2,2), bias=True, batch_size = 32):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, mode, dim, num_patches, num_heads, mlp_dim, bias=True, drop=0.0, drop_path=0.0, batch_size=32):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = Attention(mode=mode, dim=dim, num_patches=num_patches, num_heads=num_heads, bias=bias, batch_size=batch_size)
...
class Block_full(nn.Module):
    def __init__(self, dim, num_patches, num_heads, mlp_dim, bias=True, drop=0.0, drop_path=0.0, batch_size=32):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = Attention_full(dim=dim, num_patches=num_patches, num_heads=num_heads, bias=bias, batch_size=batch_size)
        self.drop_path1 = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm2 = nn.LayerNorm(dim)
        self.mlp = Mlp_full(dim=dim, hidden_dim=mlp_dim, num_patches=num_patches, drop=drop, batch_size=batch_size, bias=bias)
        self.drop_path2 = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f'before block full {x.shape}')
        x = self.block_full(x)
        print(f'after block full {x.shape}')
        x = self.norm(x)
