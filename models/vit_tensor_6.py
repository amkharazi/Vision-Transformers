...
class Attention(nn.Module):
    def __init__(
        ...
        if mode == 1:
            self.scale = self.head_dim_1 ** 0.5
            ranks = (dim[0],)
            ignore_modes = (0, 1, 3, 4)
        elif mode == 2:
            self.scale = self.head_dim_2 ** 0.5
            ranks = (dim[1],)
            ignore_modes = (0, 1, 2, 4)
        else:
            self.scale = self.head_dim_3 ** 0.5
            ranks = (dim[2],)
            ignore_modes = (0, 1, 2, 3)
        ...
        self.q_proj = TDLE(**common_kwargs)
        self.k_proj = TDLE(**common_kwargs)
        self.v_proj = TDLE(**common_kwargs)
        self.proj = TDLE(**common_kwargs)
    ...
    def forward(self, x):
        ...
        q = rearrange(
            q,
            'b n (h0 d0) (h1 d1) (h2 d2) -> b h0 h1 h2 n d0 d1 d2',
            h0=self.num_heads[0], h1=self.num_heads[1], h2=self.num_heads[2]
        )
        print(f'Q (2): {q.shape}')
...
class Mlp(nn.Module):
    def __init__(
        ...
        self.layer2 = TP(
            input_size=(batch_size, self.num_patches, hidden_dim[0], hidden_dim[1], hidden_dim[2]),
...
class Block(nn.Module):
    ...
    def forward(self, x, w):
        ...
        for m in range(self.num_modes):
            outs.append(self.attn_modes[m](x_norm))
        outs = torch.stack(outs, dim=0)

        w = w.view(-1, 1, 1, 1, 1, 1)
        attn_out = (w * outs).sum(dim=0)
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.print_mode_weights = print_mode_weights
        ...
        self.head = TP(
            input_size=(batch_size, embed_dim[0], embed_dim[1], embed_dim[2]),
            output_size=(num_classes,),
            ranks=(embed_dim[0], embed_dim[1], embed_dim[2], num_classes),
            bias=bias,
            ignore_modes=(0,),
        )

        self.mode_logits = nn.Parameter(torch.zeros(depth, num_modes))

        nn.init.trunc_normal_(self.pos_embed, std=0.02)
        if self.cls_token is not None:
            nn.init.trunc_normal_(self.cls_token, std=0.02)
        nn.init.trunc_normal_(self.mode_logits, std=0.02)
    ...
    def forward(self, x):
        ...
        for layer_idx, blk in enumerate(self.blocks):
            logits = self.mode_logits[layer_idx]
            w = F.gumbel_softmax(logits, tau=self.gumbel_tau, hard=self.gumbel_hard)
            if self.print_mode_weights:
                print(f'Layer {layer_idx} mode weights: {w.detach().cpu().numpy()}')
            print(f'before block {layer_idx} {x.shape}')
            x = blk(x, w)
            print(f'after block {layer_idx} {x.shape}')
