...
# Combination of first n Tensor Layers with last M Normal Layer
...
class Attention_tensor(nn.Module):
...
class Block_tensor(nn.Module):
    def __init__(self, dim, num_patches, num_heads, mlp_dim, bias=True, drop=0.0, drop_path=0.0, batch_size=32):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = Attention_tensor(dim=dim, num_patches=num_patches, num_heads=num_heads, bias=bias, batch_size=batch_size)
        self.drop_path1 = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm2 = nn.LayerNorm(dim)
        self.mlp = Mlp_tensor(dim=dim, hidden_dim=mlp_dim, num_patches=num_patches, drop=drop, batch_size=batch_size, bias=bias)
...
class Block_normal(nn.Module):
    def __init__(self, dim, num_heads, mlp_dim, bias=True, drop=0.0, drop_path=0.0):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = Attention_normal(dim, num_heads=num_heads, qkv_bias=bias)
        self.drop_path1 = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm2 = nn.LayerNorm(dim)
        self.mlp = Mlp_normal(dim, mlp_dim, drop=drop)
        self.drop_path2 = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.blocks_tensor = nn.ModuleList([
            Block_tensor(
                dim=embed_dim, 
                num_heads=num_heads,
                mlp_dim=mlp_dim,
            ...
            for _ in range(depth_tensor)
        ])
        
        self.blocks_normal = nn.ModuleList([
            Block_normal(
            ...
            for _ in range(depth_normal)
        ])
        self.norm = nn.LayerNorm(embed_dim[0]*embed_dim[1]*embed_dim[2])
        self.head = nn.Linear(embed_dim[0]*embed_dim[1]*embed_dim[2], num_classes)
    ...
    def forward(self, x):
        ...
        for blk in self.blocks_tensor:
            print(f'before block tensor {x.shape}')
            x = blk(x)
            print(f'after block tensor {x.shape}')
        print(f'Before rearrange to FC mode {x.shape}')
        x = rearrange(x, 'b n c h w -> b n (c h w)')
        print(f'After rearrange to FC mode {x.shape}')
        for blk in self.blocks_normal:
            print(f'before block tensor {x.shape}')
            x = blk(x)
            print(f'after block tensor {x.shape}')
        x = self.norm(x)
