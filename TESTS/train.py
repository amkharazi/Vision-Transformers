...
def get_loaders_and_meta(dataset, data_dir, batch_size, image_size):
    if dataset == 'tinyimagenet':
        ...
        train_loader, test_loader, _ = get_tinyimagenet_dataloaders(
            data_dir=data_dir, transform_train=ttr, transform_val=tv, transform_test=tt,
    ...
    if dataset == 'cifar10':
        ...
        train_loader, test_loader = get_cifar10_dataloaders(
            data_dir=data_dir, transform_train=ttr, transform_test=tt,
            batch_size=batch_size, image_size=image_size
        )
...
def main():
    ...
    parser.add_argument('--dataset', type=str, required=True,
                        choices=['tinyimagenet','cifar10','cifar100','mnist','fashionmnist','flowers102','oxford_pets','stl10'])
    parser.add_argument('--model_type', type=str, choices=['original', 'tensorized'], required=True)
    ...
    ignore_modes = tuple(int(m) for m in args.ignore_modes) if args.ignore_modes else (0,1,2) if expect_tuple else None
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if top1 > best_top1:
            best_top1 = top1
            best_epoch = epoch
            torch.save(model.state_dict(), os.path.join(model_dir, f'Best_train_epoch_{best_epoch}.pth'))
